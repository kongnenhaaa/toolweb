...
const ZALO_MO_RE = /^\[Zalo\]\s+[A-Za-z0-9_-]{8,160}$/i;
const COM_PORT_RE = /^COM[0-9]{1,4}$/i;

export function normalizeVietnamPhone(value) {
    let digits = String(value || '').replace(/\D/g, '');
    if (/^0\d{9,10}$/.test(digits)) digits = `84${digits.slice(1)}`;
    return /^84\d{9,10}$/.test(digits) ? digits : '';
}
...
export function validateGsmBridgeRequest(input) {
    ...
    const value = {
        ...
        destination: normalizeVietnamPhone(body.destination),
